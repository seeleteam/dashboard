...
class CompactOutputComponent extends React.Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    ...
    let params = {
      precision: 'ms',
      sql:`SELECT last("value") FROM "chaindb.compact.output.meter" WHERE time >= now() - (${statTimeRange}) GROUP BY time(10s), "coinbase", "networkid", "nodename" fill(null)`,
    }
    this.props.getCompactOutput(params)
  }

  getOption () {
    var SeriesArray = [] 
    var ResultData = this.props.chaindb.CompactOutputData
    ...
    const option = {
      ...
      title: {
        text: 'Stat The Metrics of chaindb.CompactOutput',
      },
      series: SeriesArray,
    }
    return option
...
export default connect(mapStateProps, mapDispatchToProps)(CompactOutputComponent)